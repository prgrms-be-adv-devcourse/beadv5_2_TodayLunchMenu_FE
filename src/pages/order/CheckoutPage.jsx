...
export default function CheckoutPage() {
  ...
  const checkoutItems = Array.isArray(location.state?.items) ? location.state.items : [];
  ...
  const paymentMethodLabel = selectedPaymentMethod === "CARD" ? "카드 결제" : "예치금 결제";
  ...
  const validate = () => {
    ...
    if (!form.receiverPhone.trim()) nextErrors.receiverPhone = "연락처를 입력해 주세요.";
    if (!form.address.trim()) nextErrors.address = "주소를 입력해 주세요.";
    if (!form.addressDetail.trim()) nextErrors.addressDetail = "상세 주소를 입력해 주세요.";
  ...
  const handleOpenConfirm = () => {
    if (!hasCheckoutItems) {
      setSubmitError("주문할 상품이 없습니다. 장바구니에서 상품을 선택해 주세요.");
      return;
    }
      ...
      <PageContainer>
        {!hasCheckoutItems ? (
          <section className="mb-6 rounded-2xl bg-amber-50 px-4 py-3 text-sm font-medium text-amber-700">
            주문할 상품 정보가 없습니다. 장바구니에서 상품을 선택한 뒤 다시 진행해 주세요.
          </section>
        ) : null}
        ...
        <div className="space-y-6 pb-32">
          <section className="rounded-[28px] bg-white/80 p-5 shadow-sm ring-1 ring-purple-100">
            <h2 className="mb-4 text-xl font-extrabold tracking-tight text-gray-900">결제수단 선택</h2>
            <div className="space-y-3">
              ...
              >
                <div className="flex items-center justify-between gap-4">
                  <div>
                    ...
                    <p className="mt-1 text-sm text-gray-500">보유한 예치금으로 즉시 결제합니다.</p>
                  </div>
              ...
              >
                <div className="flex items-center justify-between gap-4">
                  <div>
                    ...
                    <p className="mt-1 text-sm text-gray-500">PG 결제창으로 이동합니다.</p>
                  </div>
              ...
              {selectedPaymentMethod === "CARD" ? (
                <div className="rounded-2xl bg-amber-50 px-4 py-3 text-sm font-medium text-amber-700">
                  카드 결제를 선택하면 다음 화면에서 주문 생성 후 결제창이 열립니다.
                </div>
              ) : null}
            </div>
          </section>
          ...
          <section className="rounded-[28px] bg-white/80 p-5 shadow-sm ring-1 ring-purple-100">
            ...
            <div className="space-y-3">
              ...
              <div className="flex items-center justify-between text-sm">
                ...
                <span className="font-extrabold text-gray-900">{formatPrice(summary.total)}원</span>
              </div>
          ...
          <section className="rounded-[28px] bg-white/80 p-5 shadow-sm ring-1 ring-purple-100">
            <h2 className="mb-4 text-xl font-extrabold tracking-tight text-gray-900">배송 정보</h2>

            <div className="space-y-4">
              <FormField label="수령인" htmlFor="receiver" required error={errors.receiver}>
                <Input id="receiver" value={form.receiver} onChange={handleChange("receiver")} error={!!errors.receiver} />
              </FormField>
              <FormField label="연락처" htmlFor="receiverPhone" required error={errors.receiverPhone}>
                <Input id="receiverPhone" value={form.receiverPhone} onChange={handleChange("receiverPhone")} error={!!errors.receiverPhone} />
              </FormField>
              <FormField label="주소" htmlFor="address" required error={errors.address}>
                <Input id="address" value={form.address} onChange={handleChange("address")} error={!!errors.address} />
              </FormField>
              <FormField label="상세 주소" htmlFor="addressDetail" required error={errors.addressDetail}>
                <Input id="addressDetail" value={form.addressDetail} onChange={handleChange("addressDetail")} error={!!errors.addressDetail} />
              </FormField>
              <FormField label="우편번호" htmlFor="zipCode" required error={errors.zipCode}>
                <Input id="zipCode" value={form.zipCode} onChange={handleChange("zipCode")} error={!!errors.zipCode} />
              </FormField>
              <FormField label="배송 메모" htmlFor="memo">
                <Input id="memo" value={form.memo} onChange={handleChange("memo")} />
              </FormField>
            </div>
          </section>

          <section className="rounded-[28px] bg-white/80 p-5 shadow-sm ring-1 ring-purple-100">
            <h2 className="mb-4 text-xl font-extrabold tracking-tight text-gray-900">주문 상품</h2>
            <div className="space-y-4">
              {checkoutItems.map((item) => (
                <article key={item.cartId || item.productId} className="flex gap-4 rounded-2xl bg-purple-50/60 p-4">
                  ...
                  <div className="flex flex-1 items-center justify-between gap-4">
                    ...
                    <p className="text-lg font-extrabold text-violet-700">{formatPrice(item.price * item.quantity)}원</p>
                  </div>
                </article>
              ))}
            </div>
          ...
          <section className="rounded-[28px] bg-white/80 p-5 shadow-sm ring-1 ring-purple-100">
            <h2 className="mb-4 text-xl font-extrabold tracking-tight text-gray-900">결제 요약</h2>
            <div className="space-y-3">
              <div className="flex items-center justify-between text-sm">
              ...
              <div className="flex items-center justify-between text-sm">
                ...
                <span className="font-bold text-gray-900">{summary.shippingFee === 0 ? "무료" : `${formatPrice(summary.shippingFee)}원`}</span>
              </div>
              <div className="border-t border-purple-100 pt-4">
                <div className="flex items-end justify-between">
                  <span className="text-lg font-extrabold text-gray-900">총 결제 금액</span>
                  <span className="text-2xl font-extrabold tracking-tight text-violet-700">{formatPrice(summary.total)}원</span>
                </div>
              </div>
            </div>
          </section>
      ...
      <footer className="fixed bottom-0 left-0 z-40 w-full bg-white/90 backdrop-blur-xl">
        <div className="mx-auto max-w-3xl space-y-3 px-4 pb-8 pt-4 sm:px-6 lg:px-8">
          <div className="flex items-center justify-between text-sm text-gray-500">
            ...
            <span className="text-lg font-extrabold text-violet-700">{formatPrice(summary.total)}원</span>
          </div>
          <Button
            size="lg"
            className="w-full"
