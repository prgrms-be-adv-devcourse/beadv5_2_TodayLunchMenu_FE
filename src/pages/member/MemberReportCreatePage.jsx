    ...
    <PageContainer>
      <div className="mx-auto max-w-3xl">
        ...
        <div className="grid gap-6 lg:grid-cols-[0.92fr_1.08fr]">
          <section className="border border-sand bg-fog p-6 shadow-sm">
            <p className="text-xs font-bold uppercase tracking-[0.24em] text-olive">
              신고 대상
            </p>
            <div className="mt-5 flex items-center gap-4 bg-white/80 p-4 ring-1 ring-sand backdrop-blur">
              {target.image ? (
                <img
                  src={target.image}
                  alt={target.nickname}
                  className="h-16 w-16 rounded-full object-cover ring-2 ring-sand"
                />
              ) : (
                <div className="flex h-16 w-16 items-center justify-center rounded-full bg-brand text-lg font-extrabold text-white ring-2 ring-sand">
                  {getInitials(target.nickname)}
                </div>
              )}
              <div className="min-w-0">
                <div className="flex items-center gap-2">
                  <h2 className="truncate text-xl font-extrabold tracking-tight text-plum">
                    {target.nickname}
                  </h2>
                  <span className="rounded-full bg-warm px-2 py-1 text-[11px] font-bold text-plum">
                    신고 대상
                  </span>
                </div>
                ...
                <div className="mt-3 bg-dark-surface px-3 py-2 text-xs font-medium text-fog">
                  memberId: {target.memberId || '없음'}
                </div>
              </div>
            </div>

            <div className="mt-6 bg-white/80 p-5 ring-1 ring-sand">
              <p className="text-sm font-bold text-plum">신고 전에 확인해주세요</p>
              <ul className="mt-3 space-y-3 text-sm leading-6 text-olive">
          ...
          <form onSubmit={handleSubmit} className="border border-sand bg-white p-6 shadow-sm">
            <div className="space-y-6">
              <div>
                <label className="mb-3 block text-sm font-bold text-plum">
                  신고 유형 선택
                </label>
                    ...
                    return (
                      <label
                        ...
                        className={[
                          ...
                          selected
                            ? 'border-brand bg-fog shadow-sm'
                            : 'border-sand bg-fog hover:border-sand hover:bg-fog/40',
                        ].join(' ')}
                      >
                        <input
                          type="radio"
                        ...
                        <div className="flex items-start justify-between gap-3">
                          <div>
                            <p className="text-sm font-bold text-plum">{option.label}</p>
                            <p className="mt-2 text-xs leading-5 text-olive">{option.description}</p>
                          </div>
                          <span
                            className={[
                              'mt-0.5 h-5 w-5 rounded-full border-2',
                              selected ? 'border-brand bg-brand' : 'border-silver bg-white',
                            ].join(' ')}
                          />
                        </div>
                      </label>
              ...
              <div>
                <div className="mb-3 flex items-center justify-between gap-3">
                  <label htmlFor="report-reason" className="block text-sm font-bold text-plum">
                    신고 사유 작성
                  </label>
                  <span className="text-xs font-semibold text-silver">{charCount} / 1000</span>
                </div>
                <div className="overflow-hidden border border-sand bg-fog shadow-inner focus-within:border-silver focus-within:ring-2 focus-within:ring-sand">
                  <textarea
                    id="report-reason"
                    value={reason}
                    ...
                    className="min-h-[220px] w-full resize-none border-none bg-transparent px-5 py-4 text-sm leading-6 text-plum outline-none placeholder:text-silver"
                  />
                </div>
              </div>
              ...
              <div className="bg-fog px-4 py-4 text-sm leading-6 text-olive ring-1 ring-warm">
