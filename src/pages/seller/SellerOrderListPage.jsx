		...
		<>
			...
			<PageContainer>
				<section className="mb-6 flex items-end justify-between gap-4">
					<div>
						<p className="text-[10px] font-bold uppercase tracking-widest text-violet-500">Seller Orders</p>
						<h1 className="mt-1 text-2xl font-black text-gray-900">정산 대기 주문</h1>
				...
				{loading ? (
					<p className="py-16 text-center text-sm text-gray-400">주문 목록을 불러오는 중...</p>
						...
						{pagedItems.map((item) => (
							...
							>
								<div className="flex flex-col gap-3 md:flex-row md:items-center md:justify-between">
									<div>
										<p className="text-xs font-semibold text-gray-500">orderId</p>
										<p className="font-mono text-sm font-bold text-gray-900">{item.orderId || "-"}</p>
										<p className="mt-1 text-xs text-gray-500">생성일 {formatDate(item.createdAt)}</p>
									</div>

									<div className="text-left md:text-right">
										<p className="text-xs font-semibold text-gray-500">정산 대기 금액</p>
										<p className="text-lg font-black text-violet-700">{formatKRW(item.amount)}</p>
