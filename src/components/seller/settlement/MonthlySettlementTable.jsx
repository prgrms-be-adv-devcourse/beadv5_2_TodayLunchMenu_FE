...
export default function MonthlySettlementTable({
  ...
  if (items.length === 0) {
    return (
      <div className="rounded-[28px] bg-white px-6 py-10 text-center shadow-sm ring-1 ring-warm">
  ...
  return (
    <div className="space-y-4">
      <div className="flex items-center justify-between rounded-2xl bg-fog px-4 py-3 ring-1 ring-warm">
        <div>
        ...
        <p className="text-sm font-bold text-plum">{totalCount}\uAC74</p>
      </div>
      ...
      {items.map((item) => {
        ...
        return (
          <article
            ...
            className="rounded-[28px] bg-white p-5 shadow-sm ring-1 ring-warm"
          >
            <div className="flex flex-col gap-3 md:flex-row md:items-start md:justify-between">
              <div>
                ...
                <h3 className="mt-3 text-xl font-bold tracking-tight text-plum">
            ...
            <div className="mt-5 grid gap-3 rounded-2xl bg-fog px-4 py-4 text-sm md:grid-cols-3">
              <div>
                <p className="text-silver">{"\uC9C0\uAE09 \uC694\uCCAD \uC2DC\uAC01"}</p>
                <p className="mt-1 font-medium text-plum">{formatDate(item.requestedAt)}</p>
              </div>
              <div>
                <p className="text-silver">{"\uC9C0\uAE09 \uC644\uB8CC \uC2DC\uAC01"}</p>
                <p className="mt-1 font-medium text-plum">{formatDate(item.settledAt)}</p>
              </div>
              <div>
                <p className="text-silver">{"\uCD5C\uADFC \uAC31\uC2E0"}</p>
                <p className="mt-1 font-medium text-plum">{formatDate(item.updatedAt)}</p>
              </div>
            </div>
