...
export default function AiProductDraftAssistant({
  ...
  return (
    <div className="rounded-2xl bg-gradient-to-br from-violet-50 via-white to-fuchsia-50 p-4 ring-1 ring-warm">
      <div className="flex flex-col gap-4 md:flex-row md:items-center md:justify-between">
        <div>
          <p className="text-xs font-bold uppercase tracking-[0.18em] text-olive">
            AI Assist
          </p>
          <h3 className="mt-1 text-lg font-extrabold tracking-tight text-plum">
            AI로 상품 초안 만들기
          </h3>
      ...
      {helperText ? (
        <p className="mt-3 text-sm font-medium text-plum">{helperText}</p>
      ) : null}
      ...
      {hasDraft ? (
        <div className="mt-4 rounded-2xl bg-white/80 p-4 ring-1 ring-warm">
          ...
          {draft.suggestedDescription ? (
            <p className="mt-3 line-clamp-3 text-sm leading-6 text-olive">
              {draft.suggestedDescription}
            </p>
          ) : null}

          {draft.notes ? (
            <p className="mt-3 rounded-xl bg-fog px-3 py-2 text-xs font-medium text-plum">
              {draft.notes}
            </p>
          ) : null}
